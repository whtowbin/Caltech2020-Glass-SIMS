...
def Bootstrap_fit(x, y, x_err, y_err, resample_draws, InterceptFit=True, InitialGuess=[100, 1]):
    def resample(l):
...
def Eval_Conf(Fit_Param, Confidence_Bound=0.95, LineMax=200, LineInt=1, **kwargs):
    ...
    Results = pd.DataFrame(confidence_ints, columns=("neg_error_bound", "pos_error_bound"))

    # index should be added as X axis
    Results.index = x
    Results["best_fit"] = evaluted[0]
    Results["percent_error_neg"] = (Results["best_fit"] - Results["neg_error_bound"]) / np.abs(
        Results["best_fit"]
    )
    Results["percent_error_pos"] = (Results["pos_error_bound"] - Results["best_fit"]) / np.abs(
        Results["best_fit"]
    )
    return Results
...
def ODR_Bootstrap(
    ...
    **kwargs,
    ...
    param, subs = Bootstrap_fit(x, y, x_err, y_err, resample_draws, InterceptFit, InitialGuess)
...
def gauss_agv_err(concentrations, errors, cut_off=0.000001):
    ...
    def gaussian(x, sigma, avg):
        return (1 / (sigma * np.sqrt(2 * np.pi))) * (np.exp(-0.5 * ((x - avg) / sigma) ** 2))
...
def plot_Calibration_Estimates(fit_params, fit_error, Title="Calibration Line Fits"):
    ...
    Intercept_Fit_Params = gauss_agv_err(np.array(fit_params)[:, 1], np.array(fit_error)[:, 1])
